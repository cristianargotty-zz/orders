...
import { list, reset } from '../../actions/ordersitems/list';
import { success } from '../../actions/ordersitems/delete';
      ...
      <p><Link to="create" className="btn btn-primary">Create</Link></p>
        ...
        {this.props.data['hydra:member'] && this.props.data['hydra:member'].map(item =>
          <tr key={item['@id']}>
            ...
            <td>{item['itemCode'] ? itemToLinks(item['itemCode']) : ''}</td>
            <td>{item['name'] ? itemToLinks(item['name']) : ''}</td>
            <td>{item['qty'] ? itemToLinks(item['qty']) : ''}</td>
            <td>{item['gift'] ? itemToLinks(item['gift']) : ''}</td>
            <td>{item['categoryId'] ? itemToLinks(item['categoryId']) : ''}</td>
            <td>{item['categoryName'] ? itemToLinks(item['categoryName']) : ''}</td>
            <td>{item['orders'] ? itemToLinks(item['orders']) : ''}</td>
            <td>{item['price'] ? itemToLinks(item['price']) : ''}</td>
            <td>
...
const mapStateToProps = (state) => {
  return {
    data: state.ordersitems.list.data,
    error: state.ordersitems.list.error,
    loading: state.ordersitems.list.loading,
    deletedItem: state.ordersitems.del.deleted,
  };
};
