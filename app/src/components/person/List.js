      ...
      <p><Link to="create" className="btn btn-primary">Create</Link></p>
        ...
        {this.props.data['hydra:member'] && this.props.data['hydra:member'].map(item =>
          <tr key={item['@id']}>
            ...
            <td>{item['birthDate'] ? itemToLinks(item['birthDate']) : ''}</td>
            <td>{item['email'] ? itemToLinks(item['email']) : ''}</td>
            <td>{item['document'] ? itemToLinks(item['document']) : ''}</td>
            <td>{item['gender'] ? itemToLinks(item['gender']) : ''}</td>
            <td>{item['personAddress'] ? itemToLinks(item['personAddress']) : ''}</td>
            <td>
...
const mapStateToProps = (state) => {
  return {
    data: state.person.list.data,
    error: state.person.list.error,
    loading: state.person.list.loading,
    deletedItem: state.person.del.deleted,
  };
};
