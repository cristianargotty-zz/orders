...
import { list, reset } from '../../actions/personaddress/list';
import { success } from '../../actions/personaddress/delete';
      ...
      <p><Link to="create" className="btn btn-primary">Create</Link></p>
        ...
        {this.props.data['hydra:member'] && this.props.data['hydra:member'].map(item =>
          <tr key={item['@id']}>
            ...
            <td>{item['addressLine1'] ? itemToLinks(item['addressLine1']) : ''}</td>
            <td>{item['addressLine2'] ? itemToLinks(item['addressLine2']) : ''}</td>
            <td>{item['city'] ? itemToLinks(item['city']) : ''}</td>
            <td>{item['state'] ? itemToLinks(item['state']) : ''}</td>
            <td>{item['country'] ? itemToLinks(item['country']) : ''}</td>
            <td>{item['zipCode'] ? itemToLinks(item['zipCode']) : ''}</td>
...
const mapStateToProps = (state) => {
  return {
    data: state.personaddress.list.data,
    error: state.personaddress.list.error,
    loading: state.personaddress.list.loading,
    deletedItem: state.personaddress.del.deleted,
  };
};
